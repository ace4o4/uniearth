...
import { MapCanvas, MapCanvasHandle } from "@/components/dashboard/MapCanvas";
  ...
  const handleQuickAction = async (action: string) => {
    ...
    try {
      if (action === "Process-Fusion") {
        ...
        // REAL-TIME EXPORT LOGIC (Now delegates to MapCanvas for PDF)
        if (mapCanvasRef.current) {
          mapCanvasRef.current.triggerExport();
        } else {
          toast({ title: "Error", description: "Map module not ready for export.", variant: "destructive" });
        }
      } else {
        // Simulate others
